...
Usage:
    ...
    model = torch.hub.load('ultralytics/yolov5', 'custom', 'custom_yolov5s.pt')  # custom/local model
...
def _create(name, pretrained=True, channels=3, classes=80, autoshape=True, verbose=True, device=None):
    ...
    try:
        ...
        if pretrained and channels == 3 and classes == 80:
            ...
            if pretrained:
                ckpt = torch.load(attempt_download(path), map_location=cpu)  # load
...
def custom(path="runs/train/yolov5s_results/weights/best.pt", autoshape=True, _verbose=True, device=None):
    """
